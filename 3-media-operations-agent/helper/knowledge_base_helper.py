...
class KnowledgeBasesForAmazonBedrock:
    ...
    def create_or_retrieve_knowledge_base(
        ...
        if kb_id is not None:
            ...
            # Verify index is accessible before creating KB
            print(
                "========================================================================================"
            )
            print(f"Step 5.5 - Verifying Vector Index Access")
            self.verify_index_access(index_name, bedrock_kb_execution_role)
    ...
    def create_oss(
        ...
        try:
            ...
            if created:
                # Wait for data access rules to be enforced by checking OSS connectivity
                print("Waiting for data access rules to be enforced...")
                self._wait_for_oss_access(host, max_wait=180)
                # Additional wait for IAM role permissions to propagate to Bedrock service
                print("  Waiting for IAM permissions to propagate to Bedrock...")
                interactive_sleep(30)
    ...
    def _wait_for_oss_access(self, host: str, max_wait: int = 180):
        """
        Wait for OSS access policies to propagate by testing connectivity.
        
        Args:
            host: OSS collection host
            max_wait: Maximum seconds to wait (default: 180)
        """
        from opensearchpy import OpenSearch, RequestsHttpConnection
        
        print(f"  Testing OSS access (max wait: {max_wait}s)...")
        ...
        while time.time() - start_time < max_wait:
            ...
            try:
                ...
                # Try to get cluster health - this will fail if policies haven't propagated
                test_client.cluster.health()
                elapsed = time.time() - start_time
                print(f"  ✓ OSS access verified after {elapsed:.1f}s ({attempt} attempts)")
                return True
                
            except Exception as e:
                error_str = str(e).lower()
                # Expected errors during policy propagation: 404 (not found), 403/401 (auth)
                if '404' in error_str or 'not found' in error_str or 'authorization' in error_str or 'authentication' in error_str or '403' in error_str or '401' in error_str:
                    # Print status update every 30 seconds
                    current_time = time.time()
                    if current_time - last_status_time >= 30:
                        elapsed = time.time() - start_time
                        print(f"  Waiting for OSS to become accessible... ({elapsed:.0f}s elapsed)")
        ...
        # Timeout reached - print warning but continue (might still work)
    ...
    def create_vector_index(self, index_name: str):
        ...
        # Create index with retry logic for authorization errors
        max_retries = 5
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "AuthorizationException" in error_type or "authorization_exception" in str(e).lower():
                    if attempt < max_retries - 1:
                        print(f"  ⚠ Authorization error on attempt {attempt + 1}/{max_retries}")
                        print(f"  Waiting {retry_delay}s for data access policy to propagate...")
                        interactive_sleep(retry_delay)
                        retry_delay = min(retry_delay * 1.5, 120)  # Exponential backoff, max 2 minutes
                    else:
                        print(f"  ❌ Failed after {max_retries} attempts")
                        raise
                elif "RequestError" in error_type and "resource_already_exists_exception" in str(e).lower():
    ...
    def verify_index_access(self, index_name: str, bedrock_kb_execution_role: dict, max_wait: int = 180):
        """
        Verify that the data access policy has propagated by checking index accessibility.
        Uses exponential backoff with longer waits as this is a known slow operation.
        
        Args:
            index_name: Name of the OpenSearch index
            bedrock_kb_execution_role: Bedrock KB execution role (not used but kept for compatibility)
            max_wait: Maximum time to wait in seconds (default 3 minutes)
        """
        print(f"  Verifying data access policy propagation for index: {index_name}")
        print(f"  Note: This can take 1-3 minutes for AWS to propagate permissions...")
        ...
        while time.time() - start_time < max_wait:
            ...
            try:
                # Try to check if index exists and is accessible
                response = self.oss_client.indices.exists(index=index_name)
                if response:
                    # Try a simple search to verify full access
                    search_response = self.oss_client.search(
                        index=index_name,
                        body={"query": {"match_all": {}}, "size": 1}
                    )
                    print(f"  ✅ Index is accessible after {elapsed}s (attempt {attempt})")
                    # Additional wait for Bedrock service to also get permissions
                    print(f"  Waiting additional 60s for Bedrock service permissions to propagate...")
                    interactive_sleep(60)
                    return True
            except Exception as e:
                if "AuthorizationException" in str(type(e).__name__) or "403" in str(e):
                    print(f"  ⏳ Attempt {attempt}: Still waiting for permissions ({elapsed}s elapsed)")
                    print(f"     Waiting {wait_time}s before next check...")
                    time.sleep(wait_time)
                    wait_time = min(wait_time + 10, 45)  # Increase wait time, max 45s
                else:
                    print(f"  ⚠ Unexpected error: {str(e)[:100]}")
                    time.sleep(15)
        
        # Timeout reached - wait a bit more as a last resort
        print(f"  ⚠ Verification timeout after {max_wait}s")
        print(f"  Waiting final 60s buffer before proceeding...")
        interactive_sleep(60)
        return False

    @retry(wait_random_min=20000, wait_random_max=30000, stop_max_attempt_number=8)
