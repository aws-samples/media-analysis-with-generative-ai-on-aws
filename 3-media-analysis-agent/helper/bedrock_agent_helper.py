...
class AgentsForAmazonBedrock:
    ...
    def __init__(self):
        ...
        self._rek_client = boto3.client("rekognition", region_name=self._region)
    ...
    def add_lambda_action_group_with_bedrock(
        self,
        agent_name: str,
        lambda_function_name: str,
        source_code_file: str,
        ...
        if "arn:" in source_code_file:
            ...
            _lambda_arn = self.create_bedrock_lambda(
                agent_name,
                lambda_function_name,
                source_code_file,
            )
    ...
    def create_bedrock_lambda(
        ...
        lambda_role = self._create_bedrock_lambda_iam_role(agent_name)
    ...
    def _create_bedrock_lambda_iam_role(
        self,
        agent_name: str,
    ) -> object:
        """Creates an IAM role for a Lambda function built to implement an Action Group for an Agent.
        ...
        # attach Bedrock full access role
        self._iam_client.attach_role_policy(
            RoleName=_lambda_function_role_name,
    ...
    def add_lambda_action_group_with_rek(
        self,
        agent_name: str,
        lambda_function_name: str,
        source_code_file: str,
        ...
        if "arn:" in source_code_file:
            ...
            _lambda_arn = self.create_rek_lambda(
                agent_name,
                lambda_function_name,
                source_code_file,
                env_args=env_args,
    ...
    def create_rek_lambda(
        ...
        if env_args:
            # add DynamoDB Table permissions to the Lambda Function
            lambda_role = self._create_rek_lambda_iam_role(
                agent_name, dynamodb_table_name=env_args[0]
            )
            # create DynamoDB Table to be used on Lambda Code
            self.create_dynamodb(env_args[0], env_args[1], env_args[2])
            env_variables["Variables"]["cast_table"] = env_args[0]
            env_variables["Variables"]["cast_pk"] = env_args[1]

            if face_collection:
                # create face collection
                self.create_face_collection(collection_id=env_args[3])
                env_variables["Variables"]["cast_collection_id"] = env_args[3]
        else:
            lambda_role = self._create_rek_lambda_iam_role(agent_name)
    ...
    def _create_rek_lambda_iam_role(
        self,
        agent_name: str,
        dynamodb_table_name: str = None,
    ) -> object:
        ...
        # attach Rekognition full access role
        self._iam_client.attach_role_policy(
            RoleName=_lambda_function_role_name,
            PolicyArn="arn:aws:iam::aws:policy/AmazonRekognitionFullAccess",
        )

        # attach S3 full access role
        self._iam_client.attach_role_policy(
            RoleName=_lambda_function_role_name,
    ...
    def create_face_collection(
        ...
        """
        try:
            self._rek_client.create_collection(CollectionId=collection_id)
            print(f"Collection {collection_id} created.")
        except self._rek_client.exceptions.ResourceAlreadyExistsException:
            print(f"Collection {collection_id} already exists.")
        except Exception as e:
            print(f"Error creating collection: {str(e)}")
            raise

    def index_faces(self, collection_id, bucket, image_path, player):
        """
        Index faces in an image and associate them with a user.
        """
        try:
            response = self._rek_client.index_faces(
                CollectionId=collection_id,
                Image={'S3Object': {'Bucket': bucket, 'Name': image_path}},
                ExternalImageId=player,
                MaxFaces=1,
                QualityFilter="AUTO",
                DetectionAttributes=['ALL']
            )
            print(f"Face indexed from {image_path} with ExternalImageId: {player}")
            return response['FaceRecords'][0]['Face']['FaceId']
        except Exception as e:
            print(f"Error indexing face: {str(e)}")
            raise
            
    def check_user_id_exists(self, collection_id, user_id):
        """
        Check if a user_id exists in the given collection.
        """
        try:
            paginator = self._rek_client.get_paginator('list_users')
            for page in paginator.paginate(CollectionId=collection_id):
                if any(user['UserId'] == user_id for user in page['Users']):
                    return True
            return False
        except Exception as e:
    ...
    def create_user_and_associate_face(self, collection_id, player, face_id):
        """
        Create a user and associate a face with the user.
        """
        try:
            if not self.check_user_id_exists(collection_id, player):
                self._rek_client.create_user(CollectionId=collection_id, UserId=player)
                print(f"Created figure: {player}")
            
            self._rek_client.associate_faces(
                CollectionId=collection_id,
                UserId=player,
                FaceIds=[face_id]
            )
        except Exception as e:
            print(f"Error creating user or associating face: {str(e)}")
            raise
            ...
            for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
                for obj in page.get('Contents', []):
                    if obj['Key'].lower().endswith(('.png', '.jpg', '.jpeg')):
                        team, player = obj['Key'].lower().split('/')[-3:-1]
                        face_id = self.index_faces(collection_id, bucket, obj['Key'], player)
                        self.create_user_and_associate_face(collection_id, player, face_id)
        except Exception as e:
    ...
    def delete_face_collection(self, collection_id: str):
        try:
            self._rek_client.delete_collection(CollectionId=collection_id)
        except:
            pass
    ...
    def delete_dynamodb(self, table_name: str):
        try:
