"""
Film Agent for analyzing video clips and identifying films and cast members.
...
def _create_agent():
    ...
    with mcp_client:
        ...
        return Agent(
            model=bedrock_model,
            tools=gateway_tools,
            system_prompt=FilmPrompts.SYSTEM_PROMPT
        ), mcp_client

# Create agent instance
film_agent, _mcp_client = _create_agent()


def process_film(video_data: dict, s3_url: str = "") -> dict:
    """
    Main function to process film metadata using the Strands agent.
    ...
    try:        
        ...
        # Create prompt
        prompt = f"""Identify film and cast from this video:
